...
static const char *version = "ftplib Release 3.1-1 9/16/00, copyright 1996-2000 Thomas Pfau";

#ifdef WIN32
	extern int ftplib_debug = 0; // 999;
#else
	GLOBALDEF int ftplib_debug = 0; // 999;
#endif
...
GLOBALDEF int FtpRead (void *buf, int max, netbuf *nData) {
    ...
    if (nData->buf)
#ifdef WIN32
        i = readline((char *) cbuf, max, nData);
#else
        ...
        if (i != 1)
            return 0;
#ifdef WIN32
        i = net_read(nData->handle, cbuf, max);
#else
...
GLOBALDEF int FtpWrite (void *buf, int len, netbuf *nData) {
    ...
    if (nData->buf)
#ifdef WIN32
        i = writeline((char *) cbuf, len, nData);
#else
        ...
        socket_wait(nData);
#ifdef WIN32
        i = net_write(nData->handle, cbuf, len);
#else
