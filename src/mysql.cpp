...
static inline MYSQL* sql_HANDLE (Handle<Value>v) {
    if (v->IsNull()) {
        ThrowException(String::New("Handle is NULL"));
        return NULL;
    }
...
static JSVAL affected_rows (JSARGS args) {
    HandleScope scope;
    MYSQL *handle = sql_HANDLE(args[0]);
...
static JSVAL getDataRowsJson (JSARGS args) {
    ...
    MYSQL_FIELD *fields = mysql_fetch_fields(result);
#ifdef WIN32
    int *types = (int *)calloc(num_fields, sizeof(int));
...
#ifdef WIN32
	// This really sucks I need to figure out how to change this so that it can
	// be allocated with num_fields. NEED_CHANGE
	Local<String> names[50];
    int *types = (int*)calloc(num_fields, sizeof(int*));
#else
