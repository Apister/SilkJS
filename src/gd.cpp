...
#ifdef WIN32
		if (buf[0] != 0xff || buf[1] != 0xd8 || buf[2] != 0xff || (_stricmp((char *) &buf[6], "JFIF") && _stricmp((char *) &buf[6], "EXIF"))) {
#else
...
#ifdef WIN32
	gdPoint *points= new gdPoint[numPoints];
#else
