...
const char* ToAString (const v8::String::Utf8Value& value) {
...
static JSVAL compileScript (JSARGS args) {
	...
	v8::String::Utf8Value v1(args[0]->ToString());
	v8::String::Utf8Value v2(args[1]->ToString());
/*
	const char* s1 = ToAString(v1);
	const char* s2 = ToAString(v2);

	printf("args[0] = %s\r\nargs[1] = %s\r\n\r\n", s1, s2);
*/
    wrapper->script = Persistent<Script>::New(Script::New(args[0]->ToString(), args[1]->ToString()));

//	printf("exit of compileScript\r\n\r\n");
